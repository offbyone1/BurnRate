...
function currentMode() {
  ...
  const hasCodex = !!(codex && codex.planType !== null && (codex.primary || codex.secondary));
...
function currentIssues(): UsageIssue[] {
  ...
  if ((toggleState.codex || lastCodex) && codexIssue) issues.push(codexIssue);
  return issues;
}
...
async function fetchUsage(): Promise<void> {
  try {
    ...
    lastUsageJson = json;
    renderCompact(usage, lastCodex, toggleState);
    renderExpanded(usage, lastCodex, toggleState, currentIssues());
...
async function fetchCodexUsage(): Promise<void> {
  try {
    const fresh = await invoke<CodexUsage | null>("fetch_codex_usage");
    codexIssue = null;
    lastCodex = fresh; // null = Codex not configured → tiles stay empty
    ...
    console.warn("fetch_codex_usage failed:", e);
    codexIssue = describeCodexFailure(e, lastCodex);
    lastCodex = keepLastGoodOnCodexFailure(lastCodex);
    const usage = usageForRender(lastClaudeUsage());
    renderExpanded(usage, lastCodex, toggleState, currentIssues());
  }
}
...
// they figure it out".
const DBLCLICK_HINT_KEY = "burnrate-dblclick-hint-count";
...
function setupEventListeners(): void {
  ...
  document.getElementById("btn-refresh")!.addEventListener("click", () => {
    ...
    Promise.allSettled([fetchUsage(), fetchCodexUsage()]).finally(() =>
  ...
  extraToggle.checked = localStorage.getItem("burnrate-show-extra-usage") === "1";
  extraToggle.addEventListener("change", () => {
    localStorage.setItem("burnrate-show-extra-usage", extraToggle.checked ? "1" : "0");
    const usage = usageForRender(lastClaudeUsage());
    renderExpanded(usage, lastCodex, toggleState, currentIssues());
