...
const AUTO_UPDATE_KEY = "burnrate-auto-update-checks";
const LAST_UPDATE_CHECK_KEY = "burnrate-last-update-check-at";
...
async function checkForUpdates(manual: boolean): Promise<void> {
  ...
  try {
    ...
    if (!update) {
      if (manual) setStatus("BurnRate is up to date.", "success");
      return;
    }
