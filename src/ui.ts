...
export function renderCompact(
  ...
  compactView.classList.toggle('pill-empty', mode === 'none');
  compactView.classList.toggle('provider-logo-mode', mode !== 'none');
  ...
  // Helper: reset to single-row layout. The primary row still carries the
  // active provider logo so single-source mode is as identifiable as dual-mode.
  function setSingleRowVisibility(logoSvg = '', logoColor = ''): void {
    document.getElementById('pill-row-secondary')!.setAttribute('hidden', '');
    primaryLogo.innerHTML = logoSvg;
    primaryLogo.style.color = logoColor;
    if (logoSvg) primaryLogo.removeAttribute('hidden');
    else primaryLogo.setAttribute('hidden', '');
  ...
  setSingleRowVisibility(claudeBadgeSvg, CLAUDE_BRAND_COLOR);
...
function renderClaudeRatesHtml(usage: ClaudeUsageResponse): string {
  const showExtra = localStorage.getItem("burnrate-show-extra-usage") === "1";
